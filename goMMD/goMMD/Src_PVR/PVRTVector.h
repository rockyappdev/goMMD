...
static PVRTMat4 glxFrustum(
                        double left, double right, double bottom, double top, double near_val, double far_val)
{
    PVRTMat4 result;
    
    result.f[0] = (float)((2 * near_val) / (right - left));
    result.f[5] = (float)((2 * near_val) / (top - bottom));
    result.f[8] = (float)((right + left) / (right - left));
    result.f[9] = (float)((top + bottom) / (top - bottom));
    result.f[10] = (float)(-(far_val + near_val) / (far_val - near_val));
    result.f[11] = -1;
    result.f[14] = (float)(-(2 * far_val * near_val) / (far_val - near_val));
